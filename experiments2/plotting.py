...
def read_and_plot_scree(problem, file, output_dir):
    ...
    title = '{} - {}:\n'.format(ds_readable_name, problem['name']) + '{} vs Number of Components'
...
def read_and_plot_sse(problem, file, output_dir):
    ...
    title = '{} - {}: \nSSE vs Number of Clusters'.format(ds_readable_name, problem['name'])
...
def read_and_plot_acc(problem, file, output_dir):
    ...
    title = '{} - {}: \nAccuracy vs Number of Clusters'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file).set_index('k')
    p = plot_acc(title, df)
    p = watermark(p)
    p.savefig(
...
def read_and_plot_adj_mi(problem, file, output_dir):
    ...
    title = '{} - {}: \nAdj. MI vs Number of Clusters'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file).set_index('k')
...
def read_and_plot_loglikelihood(problem, file, output_dir):
    ...
    title = '{} - {}: \nLog Likelihood vs Number of Clusters'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file).set_index('k')
...
def read_and_plot_bic(problem, file, output_dir):
    ...
    title = '{} - {}: \nBIC vs Number of Clusters'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file).set_index('k')
    p = plot_bic(title, df)
    p = watermark(p)
    p.savefig(
...
def read_and_plot_sil_score(problem, file, output_dir):
    ...
    title = '{} - {}: \nSilhouette Score vs Number of Clusters'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file).set_index('k')
...
def read_and_plot_sil_samples(problem, file, output_dir):
    ...
    title = '{} - {}: \nSilhouette Samples'.format(ds_readable_name, problem['name'])
    df = pd.read_csv(file)
